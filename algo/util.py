...
from algo.account import Account
...
def createAsset(client: AlgodClient, asset: Asset, account: Account = None) -> int:
    if account is None:
        raise Exception('请指定官方账号')

    # this random note reduces the likelihood of this transaction looking like a duplicate
    random_note = bytes(randint(0, 255) for _ in range(20))
